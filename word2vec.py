...
sentences = []
with open('datawork/stocks.csv') as csv:
    csv_reader = reader(csv, delimiter='|')
    for row in csv_reader:
        # log.info(row[0] + ' ' + row[1])
        sentences.append(row[0] + ' ' + row[1].rsplit('.')[0])
...
print(model['GameStop'])

# print(model.wv.similar_by_word(word='GameStop'))
# print(model.wv.most_similar(positive=['GameStop'], topn=5))
# model.wv.most_similar(positive=['Sudan'], negative=['UN'], topn=5)
# model.wv.most_similar(positive=['Sudan', 'UN'], topn=5)
print(model.wv.similarity('GameStop', 'GME'))
# print(model.wv.similarity('peace', 'Sudan'))
